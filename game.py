...
class board:        # used for representing the initial board, does not include the animals
    def __init__(self,empty_board:list):
...
class State():              #object with one state of the game
    def __init__(self,board_,p1_animals,p2_animals):
        self.board = board_
        self.animals = {1:p1_animals,2:p2_animals}      #made for selecting animals easier
    ...
    def __repr__(self):                 #used for converting into a string using the repr() function, may delete this because the code isn't very readable, a display function should do the trick
        return "\n".join("".join(str(a[(x,y)]) if (x,y) in (a:=self.animals[1]) else str(a[(x,y)]) if (x,y) in (a:=self.animals[2]) else self.board.empty_board[y][x] for x in range(7)) for y in range(9))
